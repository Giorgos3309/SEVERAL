...
print_statistics(unsigned int received, struct timespec start, struct timespec end)
{
    ...
    double megabytes = (double)(received*8.0) / 1000000.0;
...
void* UDPservice(void* clnt){
    ...
    totalrecv[cl_no] = 0;
    //printf("totalrecv:%d  packet_counter:%d" , totalrecv[cl_no] , packet_counter[cl_no]);
...
//	printf("\nSIZE OF UNSIGNED LONG%d",sizeof(unsigned long long));
    jitters[cl_no]=0;
    sdjitter[cl_no]=0;
    while(1)
    {
...
//		elapsed =(timejitter.tv_nsec - tmptimejitter.tv_nsec);
...
//	printf("\nO counter einai %d kai to jitters %llu\n",TEMPcounter,jitters[cl_no]);
    ...
    //close(usock[s_no]);//to do i must close it in main thread
    return NULL;
}
...
//        printf("received message:%s\n",buffer);
    ...
    //pthread_join(cl_tid[0],NULL);
    client_available[cl_no] = 1;
    close(acc);
    printf("\n--------CLIENT%d------------\n" , cl_no);
    printf("Packets send: %u\n", data->seq_number);
    printf("Packets received: %d\n",packet_counter[cl_no]-1);
    printf("To average jitter einai %llu nanosec\n",jitters[cl_no]/(packet_counter[cl_no]-1));
//    printf("To SD jitter prin to sqrt einai %llu nanosec\n",sdjitter[cl_no]/(packet_counter[cl_no]-1));
    double SDforSQRT=(double) (sdjitter[cl_no]/(packet_counter[cl_no]-1));
     printf("To SD of jitter einai %lf nanosec\n",sqrt(SDforSQRT));
